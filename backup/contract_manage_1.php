<?php 
    ...
    $total_overdue_count = $contract_data['total_overdue_count'];
    $total_overdue_amount = $contract_data['total_overdue_amount'];
    ...
    $customer_id = 0;
    $customer_data = null;
    $product_name = ''; $agreement_date = ''; $loan_amount = ''; $repayment_method = '';
    $monthly_installment = ''; $loan_date = ''; $maturity_date = ''; $manager = '';
    $collateral_address = ''; $bank_name = ''; $account_number = ''; $account_holder = '';
    $interest_rate = ''; $overdue_interest_rate = ''; $prepayment_rate = ''; $next_due_date = '';
    $status = ''; $rate_change_date = ''; $new_interest_rate = ''; $new_overdue_rate = '';
...
.modal-content { background-color: #fefefe; margin: 5% auto; padding: 25px; border: 1px solid #888; width: 400px; height: 900px; border-radius: 8px; display: flex; flex-direction: column; }
.close-button { color: #aaa; float: right; font-size: 28px; font-weight: bold; }
.close-button:hover, .close-button:focus { color: black; text-decoration: none; cursor: pointer; }
.memo-section { display: flex; flex-direction: column; height: 100%; }
.memo-list { flex-grow: 1; overflow-y: auto; border: 1px solid #ddd; padding: 10px; margin-bottom: 15px; }
.memo-item { border-bottom: 1px solid #eee; padding: 10px 5px; }
.memo-item:last-child { border-bottom: none; }
.memo-actions { float: right; }
.memo-text { margin: 5px 0; font-size: 14px; }
.memo-meta { font-size: 12px; color: #6c757d; }
.memo-form-container { flex-shrink: 0; }
.memo-form-options { grid-template-columns: 1fr 1fr; }
...
<div class="search-form-container">
    <form action="contract_manage.php" method="get">
        <div class="search-form-flex">
            ...
            <div class="form-col"><label>상태</label><select name="status"><option value="valid" <?php if ($search_params['status'] == 'valid') echo 'selected'; ?>>유효(정상,연체)</option><option value="" <?php if ($search_params['status'] == '') echo 'selected'; ?>>전체</option><?php foreach ($statuses as $key => $value): ?><option value="<?php echo $key; ?>" <?php if ($search_params['status'] == $key) echo 'selected'; ?>><?php echo $value; ?></option><?php endforeach; ?></select></div>
            <div class="form-col"><label>약정일</label><select name="agreement_date"><option value="">전체</option><?php foreach ($due_days as $day): ?><option value="<?php echo $day; ?>" <?php if ($search_params['agreement_date'] == $day) echo 'selected'; ?>><?php echo $day; ?>일</option><?php endforeach; ?></select></div>
    ...
    <table class="contract-list-table">
        <thead>
            <tr>
                ...
                <th>고객번호</th><th>계약번호</th><th>고객명</th><th>상품명</th><th>대출금액</th><th>현재잔액</th><th>대출일</th><th>만기일</th><th>약정일</th><th>이자율</th><th>최근입금일</th><th>다음입금예정일</th><th>상태</th><th style="width: 280px;">Actions</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr data-contract-id="<?php echo htmlspecialchars($contract['id']); ?>" <?php echo $row_style; ?>>
                ...
                <td>
                    ...
                    if (!empty($assigned_codes)) {
                        ...
                        foreach ($assigned_codes as $code) {
                            echo '<span class="badge bg-info text-dark" style="font-size: 0.75em; padding: 3px 6px; border-radius: 4px;" title="' . htmlspecialchars($code['name']) . '">' . htmlspecialchars($code['code']) . '</span>';
                        }
                        echo '</div>';
                    }
                    ?>
                </td>
                <td><a href="customer_detail.php?id=<?php echo $contract['customer_id']; ?>"><?php echo htmlspecialchars($contract['id']); ?></a></td>
                ...
                <td style="text-align: right;"><?php echo number_format($contract['loan_amount']); ?></td>
                <td style="text-align: right; font-weight: bold;"><?php echo number_format($contract['current_outstanding_principal']); ?></td>
                ...
                <td><?php echo htmlspecialchars($contract['interest_rate']); ?>%</td>
                ...
                <td class="action-buttons">
                    <a href="expected_interest_view.php?contract_id=<?php echo $contract['id']; ?>" class="btn btn-xs view_btn" onclick="window.open(this.href, '_blank', 'width=1200,height=700'); return false;">예상이자</a>
                    ...
                    <a href="transaction_ledger.php?contract_id=<?php echo $contract['id']; ?>" class="btn btn-xs view_btn">원장</a>
                    <a href="sms.php?contract_id=<?php echo $contract['id']; ?>&customer_id=<?php echo $contract['customer_id']; ?>" class="btn btn-xs view_btn" onclick="window.open(this.href, '_blank', 'width=1200,height=800'); return false;">SMS</a>
                    <?php if (isset($_SESSION['permission_level']) && $_SESSION['permission_level'] == 0): ?>
                    <a href="collection_manage.php?contract_id=<?php echo $contract['id']; ?>" class="btn btn-xs add_btn">입금</a>
                    ...
                    <a href="../process/reset_contract_data.php?id=<?php echo $contract['id']; ?>" class="btn btn-xs del_btn" onclick="window.open(this.href, 'reset_window', 'width=600,height=400'); return false;">초기화</a>
                    <?php endif; ?>
                </td>
            </tr>
            <?php endforeach; else: ?>
            <tr><td colspan="15" style="text-align: center;">검색된 계약이 없습니다.</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
</div>
...
<div id="add_contract_section" class="form-container" style="<?php if (!$customer_data && !$update) echo 'display:none;'; ?>">
    <h2><?php echo $update ? '계약 수정' : '신규 계약 추가'; ?></h2>
    ...
    <div id="selected_customer_info" <?php if (!$customer_data) echo 'style="display:none;"'; ?>>
        ...
        <div id="customer_details_box">
            ...
            <p><strong>고객명:</strong> <?php echo htmlspecialchars($customer_data['name']); ?></p>
            <p><strong>주민번호:</strong> <?php echo htmlspecialchars($customer_data['resident_id_partial']); ?></p>
            <p><strong>연락처:</strong> <?php echo htmlspecialchars($customer_data['phone']); ?></p>
            <?php endif; ?>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (showAddFormBtn) {
        showAddFormBtn.addEventListener('click', function() {
            ...
            addContractSection.scrollIntoView({ behavior: 'smooth' });
        });
    }
