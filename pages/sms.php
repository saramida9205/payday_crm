...
<div class="sms-layout-container">
    ...
    <div class="sms-layout-column" style="display: flex; flex-direction: column; gap: 20px;">
        ...
        <div id="step1-card" class="card active">
            <div class="card-header">
                <h5><i class="fas fa-users"></i> 1단계: 발송 대상 선택</h5>
            </div>
            <div class="card-body">
        ...
        <div id="step2-card" class="card step-card">
            <div class="card-header">
                <h5><i class="fas fa-file-alt"></i> 2단계: 템플릿 선택</h5>
            </div>
            <div class="card-body">
    ...
    <div class="sms-layout-column">
        <div id="step3-card" class="card step-card">
            <div class="card-header">
                <h5><i class="fas fa-eye"></i> 3단계: 미리보기</h5>
            </div>
            <div class="card-body">
                <div id="manual-message-area" style="display:none; margin-bottom: 15px;">
                    ...
                    <div style="position: relative; width: 80%; margin-bottom: 15px;">
                        <textarea id="manual-message-input" class="form-control" rows="6" spellcheck="false" style="width: 100%; padding: 10px; line-height: 1.6; font-weight: normal; font-size: 16px; font-family: 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; padding-bottom: 25px;"></textarea>
                        <div id="char-counter" style="position: absolute; bottom: 10px; right: 15px; font-size: 12px; color: #6c757d;">0 / 2000자</div>
                    </div>
                </div>
                <div style="margin-bottom: 15px;">
                    <label for="future-date-input" style="font-size: 14px;">미래일자 (선택사항, [미래일자...] 변수용)</label>
                    <input type="date" id="future-date-input" class="form-control" style="width: 100%;">
                </div>
    ...
    <div class="sms-layout-column">
        <div id="step4-card" class="card step-card">
            <div class="card-header">
                <h5><i class="fas fa-paper-plane"></i> 4단계: 발송하기</h5>
            </div>
            <div class="card-body">
                    ...
                    <div class="sms-template-box" style="height: auto; min-height: 150px; border-style: dashed;">
                        <form action="../process/sms_process.php" method="post">
                            ...
                            <div class="input-group" style="margin:0; position: relative;">
                                ...
                                <p style="font-size: 11px; color: #6c757d; margin-top: 5px;">사용 가능 변수: [고객명], [약정일], [약정일이자], [이자납부계좌], [연락처], [회사연락처], [오늘이자], [오늘총이자], [오늘완납금액], [현재대출잔액], [최초대출원금], [유예약정금], [미수취비용], [대출일], [오늘날짜], [오늘납부내역], [미래일자], [미래일자총이자], [미래일자완납금액]</p>
                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (isSingleMode) {
            document.getElementById('step1-card').scrollIntoView({
                behavior: 'smooth',
                block: 'start'
            });
        }
        ...
        if (generatePreviewBtn) {
            generatePreviewBtn.addEventListener('click', async function() {
                ...
                const futureDate = futureDateInput ? futureDateInput.value : '';
                ...
                let futureDataMap = {};
                if (futureDate) {
                    const contractIds = selectedContracts.map(cb => cb.value);
                    try {
                        const response = await fetch('../process/get_future_interest.php', {
                            method: 'POST',
                            headers: {
                        ...
                        if (result.success) {
                            futureDataMap = result.data;
                        } else {
                            alert('미래일자 계산 중 오류가 발생했습니다: ' + result.message);
                            return;
                        }
                    } catch (error) {
                        console.error('Error fetching future interest:', error);
                ...
                selectedContracts.forEach(contractCb => {
                    ...
                    const futureInfo = futureDataMap[contractId] || {};
                    ...
                    const replacements = {
                        ...
                        '[미래일자]': futureDate || '',
                        '[미래일자총이자]': futureInfo.future_total_interest ? new Intl.NumberFormat('ko-KR').format(futureInfo.future_total_interest) + '원' : '-',
                        '[미래일자완납금액]': futureInfo.future_payoff_amount ? new Intl.NumberFormat('ko-KR').format(futureInfo.future_payoff_amount) + '원' : '-'
                    };
        ...
        if (modalTextarea && modalCharCounter) {
            ...
            new MutationObserver(updateModalCounter).observe(modalTextarea, {
                attributes: true,
