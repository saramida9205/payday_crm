<?php 
    ...
    $active_tab = $_GET['tab'] ?? 'withdrawals';
    ...
    if ($active_tab == 'withdrawals') {
        ...
        foreach($deposit_details as $row) {
            $deposit_totals['count']++; $deposit_totals['total_deposit'] += $row['total_deposit']; $deposit_totals['expense'] += $row['expense']; $deposit_totals['interest'] += $row['interest']; $deposit_totals['principal'] += $row['principal'];
        }
    }
?>
...
<style>
    ...
    .tab-nav a { display: inline-block; padding: 10px 15px; border: 1px solid transparent; border-bottom: 0; margin-bottom: -2px; text-decoration: none; color: #495057; }
    .tab-nav a.active { color: #007bff; border-color: #dee2e6 #dee2e6 #fff; border-radius: 0.25rem 0.25rem 0 0; font-weight: bold; }
    .tab-content { display: none; }
    .tab-content.active { display: block; }
    .clickable-row { cursor: pointer; }
    .clickable-row:hover { background-color: #f8f9fa; }
    .modal { display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.5); }
    .modal-content { background-color: #fefefe; margin: 10% auto; padding: 25px; border: 1px solid #888; width: 80%; max-width: 700px; border-radius: 8px; }
...
<div id="withdrawals" class="tab-content <?php echo $active_tab == 'withdrawals' ? 'active' : ''; ?>">
    ...
    <div class="table-container">
        <table>
            <thead>
                <tr>
                    <th>회원번호</th><th>계약번호</th><th>현재상태</th><th>상품명</th><th>고객명</th><th>계약일</th><th>만기일</th><th>약정일</th><th>차기상환일</th><th>이율</th><th>출금금액</th><th>송금계좌</th><th>조회기준일 잔액</th>
                </tr>
            </thead>
            <tbody>
            <?php if (empty($withdrawal_details)): ?>
                <tr><td colspan="13" style="text-align: center;">조회된 내역이 없습니다.</td></tr>
            <?php else: 
                foreach ($withdrawal_details as $row): 
...
<div id="deposits" class="tab-content <?php echo $active_tab == 'deposits' ? 'active' : ''; ?>">
    ...
    <div class="table-container">
        <table>
            <thead>
                <tr>
                    <th>회원번호</th><th>계약번호</th><th>고객명</th><th>이전입금일</th><th>현재입금일</th><th>약정일</th><th>입금액</th><th>비용상환</th><th>이자상환</th><th>원금상환</th><th>부족금액</th><th>현재잔액</th>
                </tr>
            </thead>
            <tbody>
            <?php if (empty($deposit_details)): ?>
                <tr><td colspan="12" style="text-align: center;">조회된 내역이 없습니다.</td></tr>
