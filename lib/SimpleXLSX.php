...
class SimpleXLSX
{
    ...
    /*
        ...
        private $number_formats = array(
            ...
            0x25 => '%1.0f',    // دیکد"#,##0;(#,##0)",
            0x26 => '%1.0f',    // دیکد"#,##0;(#,##0)",
            0x27 => '%1.2f',    // دیکد"#,##0.00;(#,##0.00)",
            0x28 => '%1.2f',    // دیکد"#,##0.00;(#,##0.00)",
            0x29 => '%1.0f',    // دیکد"#,##0;(#,##0)",
            0x2a => '$%1.0f',   //"$#,##0;($#,##0)",
            0x2b => '%1.2f',    // دیکد"#,##0.00;(#,##0.00)",
            0x2c => '$%1.2f',   //"$#,##0.00;($#,##0.00)",
            0x30 => '%1.0f');   //"##0.0E0";
        // }
    */
...
*/
    ...
    public function getEntryXML($name)
    {
        if ($entry_xml = $this->getEntryData($name)) {
            ...
            // if (defined('PHP_VERSION_ID') && PHP_VERSION_ID < 80000 && LIBXML_VERSION < 20900 && function_exists('libxml_disable_entity_loader')) {
    ...
    */
    ...
    public function readRowsEx($worksheetIndex = 0, $limit = 0)
    {
        if (!$this->rowsExReader) {
            ...
            throw new \Exception('SimpleXLSXEx not available');
        }
